...
Adds Parallel AI MCP integration to Gandalf for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Gandalf scheduler for result polling

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Gandalf already set up and running
3. Gandalf running locally
...
Add Parallel AI MCP servers after the gandalf server:
```typescript
const mcpServers: Record<string, any> = {
  gandalf: ipcMcp
};
...
### 4. Add Usage Instructions to CLAUDE.md
...
3. Create a polling scheduled task using `mcp__gandalf__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__gandalf__send_message
   4. Use mcp__gandalf__complete_scheduled_task to mark this task as done
...
- Check task runs: `tail -f logs/gandalf.log | grep "scheduled task"`
...
2. Revert changes to src/agent-runner.ts
